...
############################################################################## Таблица с данными
        ...
        self.tree.column('wages', width=300, anchor=tk.CENTER)
...
############################################################################## ОСНОВНЫЕ МЕТОДЫ
    ...
    def records(self, name, phone, email, wages):
        self.db.insert_data(name, phone, email, wages)
        self.view_records()


    # Обновление (изменение ) данных
    def update_record(self, name, phone, email, wages):
...
##############################################################################
        ...
        label_email = tk.Label(self, text='Заработная плата: ')
        label_email.place(x=50, y=110)
        # Виджеты ввода
...
class DB:
    ...
    def insert_data(self, name, phone, email, wages):
        self.cur.execute(""" INSERT INTO users (name, phone, email, wages)
                             VALUES(?, ?, ?)""", (name, phone, email, wages))
        self.conn.commit()
##############################################################################
